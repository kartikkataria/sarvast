...
export async function GET(request: NextRequest) {
  ...
  const tokenRes = await fetch("https://oauth2.googleapis.com/token", {
    ...
    body: new URLSearchParams({
      ...
      redirect_uri: `${origin}/api/connections/google_ads/callback`,
  ...
  if (upsertError) {
    console.error("[Google Ads callback] upsert failed:", upsertError);
    return NextResponse.redirect(`${origin}/connections?error=save_failed`);
  }

  return NextResponse.redirect(`${origin}/connections?connected=google_ads`);
}
